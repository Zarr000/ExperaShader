...
// Original PBR BRDF helpers: GGX, Smith, Schlick Fresnel.
...
vec3 PBR_Lit(vec3 albedo, float metallic, float roughness, float ao, vec3 N, vec3 V, vec3 L, vec3 lightColor, float lightIntensity) {
    ...
    // Apply AO to diffuse and keep spec mostly un-occluded for plausible results.
    vec3 radiance = lightColor * lightIntensity;
    vec3 color = brdf * radiance * NoL;
